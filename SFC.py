...
for t in range(1, T + 1):
    ...
    for j in range(1, N_ITER + 1):
        ...
        if i[v.L, 0] == 0:
            ...
            i[v.Gamma, j] = (i[v.BB, 0] + i[v.L, 0] - i[v.D, 0] - i[v.S, 0]) / i[v.L, 0]
        ...
        i[v.CT, j] = max(
            0,
            par[p.ay] * (i[v.W, j] + i[v.M, j]) + par[p.av] * (i[v.DH, 0] + i[v.S, 0]),
        )
        ...
        i[v.nCT, j] = min(
            i[v.KFC, 0],
            max(0, (i[v.CT, j] + i[v.GT, j]) / (i[v.pC, j] * i[v.beta, j] * par[p.k])),
        )
        i[v.nKT, j] = min(
            i[v.KFK, 0],
        ...
        if i[v.W, j] == 0 or (i[v.nK, j] + i[v.nQ, j]) == 0 or i[v.omega, j] == 1:
            ...
            i[v.WFK, j] = i[v.W, j] * (i[v.nK, j] + i[v.nQ, j]) / (1 - i[v.omega, j])
        ...
        i[v.BG, j] = (i[v.BG, 0] + i[v.G, j] + i[v.M, j] - i[v.T, j]) / (1 - i[v.rB, j])
...
print(
    tabulate(
        numpy.hstack((numpy.array([["D", "S", "L", "B", "K", "V"]]).T, bs[:, :, t])),
        floatfmt=".4f",
        headers=["", "H", "FC", "FK", "B", "G", ""],
    )
)
...
# %%
# print(tabulate(numpy.hstack((numpy.array([[e.name for e in v]]).T, s))))
# %%
tol = numpy.zeros(N_ITER - 1)
for j in range(N_ITER - 1):
    tol[j] = numpy.log10(((i[:, j] - i[:, j + 1]) ** 2).sum())
plt.plot(tol)
# %%
plt.plot(s[v.Y, 25:-1])
# %%
plt.plot(((s[v.C, :] + s[v.G, :]) / s[v.pC, :])[25:-1])
# %%
plt.plot(s[v.g, 25:-1])
# %%
